...
- **AI Assist Log** — turns AI *use* into AI *evaluation*, and makes it fast enough to take seriously. Logging is a **two-field quick-log** (the AI's **claim** + **how you'll check it**), with one-tap structured picks (claim type, whether a source was given) and an optional deeper AUDIT. **Logging AI is one tap.** A small **🤖 button sits next to every text-input area** (Founder & Opportunity, Canvases, Assumptions, Experiments, Evidence, VP Auction, Pivots, Concept Check, Decision Journal). Tap it, optionally type one line, and it's logged — **no required fields, no audit to fill in at capture time**. Verification is a separate, later step and is the *only* thing that requires effort: either **link the claim to an assumption** (it auto-verifies when you test that assumption) or set the status with **one click** (✅/❌/✏️). The full AUDIT is always available but entirely optional. **Link a claim to an assumption and it auto-verifies**: when that assumption tests Supported/Refuted, the log flips to Verified/Rejected with no extra step. Status is set with **one-click buttons** (✅ Verified / ❌ Rejected / ✏️ Modified / ⏳ Unverified), and an **"N unverified" nudge** plus a verification-rate bar keep it honest. The round score's **AI-verification** component rewards the share you actually evaluated — **not** usage — and is neutral (`n/a`) for teams that don't use AI.
