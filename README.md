...
- **Founder & Team** — a realistic model of the founding team:
  - **Founder-time per round** — the founder gets a limited number of hours each round (by difficulty) that refills as the founder keeps working, split between **building** (experiments) and **training** skills.
  - **Skills** — seven structured capabilities (Customer Research, Design & Prototyping, Technical/Engineering, Sales & Growth, Finance & Unit Economics, Operations, Responsible Innovation), each with a defined meaning and a real effect (raises its matching performance dimension). **Train** a skill for founder-hours, and **undo** a training for a refund if you change your mind.
  - **Hiring** — founders can't do everything. When a needed skill is weak, **hire a specialist part-time or full-time** (upfront cost + an ongoing salary charged each round) to raise your **effective** skill (founder level + hires, capped at 5). Let a hire go to stop the salary. The page flags which skills the **current round** leans on so you know when to train vs. hire.
