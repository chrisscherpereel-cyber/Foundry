...
- **AI Assist Log** — turns AI *use* into AI *evaluation*, and makes it fast enough to take seriously. Logging is a **two-field quick-log** (the AI's **claim** + **how you'll check it**), with one-tap structured picks (claim type, whether a source was given) and an optional deeper AUDIT. **Every AI use gets a full — but fast — audit, and *where* it was used is always recorded.** A small **🔵 AI badge (a colored dot) sits next to every text-input area** (Founder & Opportunity, Canvases, Assumptions, Experiments, Evidence, VP Auction, Pivots, Concept Check, Decision Journal). Because each button knows its own field, pressing it **auto-records the exact field/section** (e.g. "Concept check: Behavior vs. opinion", "Canvas: Value Proposition Canvas", "Evidence Ledger") — no need to pick where. The form then captures the **prompt**, the **AI's output**, and **how the AI was used** (brainstorm / draft / summarize / critique / analyze / plan), then an **audit answered by dropdowns** — hidden assumptions, unsupported claims, sources/data, and how you'll verify — with free-text notes only if you want them. The only field you must type is the AI's output. **Verification stays the point:** link the claim to an assumption and it **auto-verifies** when you test it, or set the status with **one click** (✅/❌/✏️), and only verified evaluation (not usage) feeds the round score. **Any still-unverified AI use shows up on the sidebar to-do list** until it's checked. **Link a claim to an assumption and it auto-verifies**: when that assumption tests Supported/Refuted, the log flips to Verified/Rejected with no extra step. Status is set with **one-click buttons** (✅ Verified / ❌ Rejected / ✏️ Modified / ⏳ Unverified), and an **"N unverified" nudge** plus a verification-rate bar keep it honest. The round score's **AI-verification** component rewards the share you actually evaluated — **not** usage — and is neutral (`n/a`) for teams that don't use AI.
