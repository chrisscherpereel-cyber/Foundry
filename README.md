...
- **Round Control** — set the current round manually; toggle Strict round mode; and tune the **Economy & balance** (founder-hours per round for all teams, training cost per level, and part-/full-time hire boost, upfront, and salary) without editing code. Also shows this round's objectives/concepts/task and the schedule map.
...
- **Founder & Team** — a realistic model of the founding team:
  - **Weekly founder time & allocation** — the founder works a long week (40–80 hours by difficulty), but **productivity drops past 40 hours**, so effective time grows slowly. Time **resets each round** (unused hours are lost). A **time-allocation planner** shows exactly where the week goes — **admin & overhead** and **managing hires** are fixed, and the team splits the remaining **discretionary** hours between **building the business** and **learning/training** with a slider they can **re-allocate every round** based on results. A **Pareto chart** visualizes the founder's buckets plus each hired team member's hours. (Setting the allocation is a Round-1 deliverable, and "founder & team time allocation" is a Round-1 concept.)
  - **Skills** — seven capabilities (Customer Research, Design & Prototyping, Technical/Engineering, Sales & Growth, Finance & Unit Economics, Operations, Responsible Innovation), each with a defined meaning and a real effect (raises its matching performance dimension). Founders improve them three ways: **training** (spend founder-hours; **undoable** for a refund), **hiring**, and **learning by doing** — finishing a round's own work grows the skills that round leaned on.
