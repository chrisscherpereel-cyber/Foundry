...
- **Founder & Team** — a realistic model of the founding team:
  - **Weekly founder effort & hours** — one simple control: choose how hard the founder works, up to a **hard cap of 80 hours** (the first 40 are fully productive; each hour past 40 counts for half, so 80 raw ≈ 60 *effective*). **Admin & overhead** and **managing hires** come off the top; the rest is a single pool the team spends **as they act** — on experiments (building) and training. A **Pareto chart of actual usage** (Admin, Managing, Building, Training, Hiring, Available) updates live and always sums to the productive week, so hours are easy to read and manage. Hours **reset each round** (unused hours are lost). (Setting effort is a Round-1 deliverable; "founder & team time allocation" is a Round-1 concept.)
