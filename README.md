...
- **Round Briefing** — the starting point each round: learning objectives, concepts, tasks, and a **shaded completion checklist** ("To finish Round N — x/y complete") where amber items must still be done and green items are done. It also spells out **what must change this round** vs. **what can remain** (carried forward), and what's locked. Includes the full round-by-round arc.
  ...
  - **Sidebar progress tracker** — the team sidebar shows a live **"Round N to-do: x/y done"** bar with an expander listing exactly what's still open and on which tool (including anything carried over **and any unread Inbox messages** — clearing the inbox is itself a to-do item), so from any page the team can see what's left this round.
...
- **AI Assist Log** — turns AI *use* into AI *evaluation*, and makes it fast enough to take seriously. Logging is a **two-field quick-log** (the AI's **claim** + **how you'll check it**), with one-tap structured picks (claim type, whether a source was given) and an optional deeper AUDIT. **Every AI use gets a full — but fast — audit.** A small **(AI) button sits next to every text-input area** (Founder & Opportunity, Canvases, Assumptions, Experiments, Evidence, VP Auction, Pivots, Concept Check, Decision Journal). Tapping it opens one consistent form that captures the **prompt**, the **AI's output**, and **how the AI was used** (brainstorm / draft / summarize / critique / analyze / plan), then an **audit answered by dropdowns** — hidden assumptions, unsupported claims, sources/data, and how you'll verify — with free-text notes only if you want them. The only required field is the AI's output. **Verification stays the point:** link the claim to an assumption and it **auto-verifies** when you test that assumption, or set the status with **one click** (✅/❌/✏️), and only verified evaluation (not usage) feeds the round score. **Link a claim to an assumption and it auto-verifies**: when that assumption tests Supported/Refuted, the log flips to Verified/Rejected with no extra step. Status is set with **one-click buttons** (✅ Verified / ❌ Rejected / ✏️ Modified / ⏳ Unverified), and an **"N unverified" nudge** plus a verification-rate bar keep it honest. The round score's **AI-verification** component rewards the share you actually evaluated — **not** usage — and is neutral (`n/a`) for teams that don't use AI.
