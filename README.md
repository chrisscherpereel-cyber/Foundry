...
- **Founder & Team** — a realistic model of the founding team:
  - **Weekly founder effort & allocation** — the team chooses how hard the founder works, up to a **hard cap of 80 hours**; the first 40 are fully productive and each hour past 40 counts for half, so pushing to 80 yields ~60 *effective* hours (diminishing returns on all work). Time **resets each round** (unused hours are lost). A **planner** shows where the week goes — **admin & overhead** and **managing hires** come off the top, and the team splits the remaining **discretionary effective hours** between **building** and **learning/training** with sliders they can **re-allocate every round**. A **Pareto chart** visualizes the founder's buckets plus each hired member's hours. (Setting effort/allocation is a Round-1 deliverable; "founder & team time allocation" is a Round-1 concept.)
