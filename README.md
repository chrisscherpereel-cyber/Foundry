...
- **Round Scores** — one automated **0–100 grade per team** for the work they committed in a round. It blends four components — **commitment/completion** (share of the round's decisions + concept-checks done), **evidence quality**, **business-model coherence**, and **concept coverage** — then subtracts a **risk penalty** for important, still-untested assumptions. A **sensitivity panel** lets you set each component's **weight** (normalized) and a **strictness dial** (lenient ↔ harsh, a curve on the final score) with one-click reset. The table ranks all teams live and shows whether each committed; you can **preview/edit and email** any team its score breakdown, or send to all.
...
- **Round Briefing** — the starting point each round: learning objectives, concepts, tasks, and a **shaded completion checklist** ("To finish Round N — x/y complete") where amber items must still be done and green items are done. It also spells out **what must change this round** vs. **what can remain** (carried forward), and what's locked. Includes the full round-by-round arc.
  - **Decision deadline & commit** — a "Commit your round" panel tells the team **when this round's decisions are due** (taken from the schedule's next-round advance time) and how long is left; if the instructor hasn't scheduled an advance time it **says so explicitly** ("No deadline is set — decisions stay open until your instructor advances the round"). It lists any decisions still open, then lets the team **commit** their round's work — and **withdraw the commitment any time before the deadline** to keep editing. Once the deadline passes the round **locks** and the last committed work is what's scored. **Commit status is always visible in the team sidebar** — a ✅ committed / ⚠️ not-committed banner (with the time remaining) plus a matching badge on the Round Briefing entry — so teams can see at a glance whether they still need to commit.
...
- **Concept Check** — guarantees every concept introduced that round is covered: each concept must be either handled by a decision (a tool action) or answered here in a sentence or two. Unanswered concepts from earlier rounds are carried forward until done.
