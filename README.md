...
- **Schedule & Timing** — define **how many rounds** the simulation runs (default 15) and pack the curriculum into them however your class meets. Each round can cover **one OR more pieces of material**, so all 15 topics can be covered even in, say, 7 rounds. For each round you can **add material**, **remove it**, or **move it to another round**; anything not placed sits in an **Unassigned material** pool (with a coverage counter) until you slot it in. Shrinking the round count never loses material — it moves onto the last round. You can also set a **date/time** for each round to begin; the sim auto-advances the next time the app is opened after that time (no always-on server). One-click reset to the default 15-round order.
...
- **Cover all material in fewer rounds** — a round can hold several topics, so the full 15-topic curriculum fits into however many rounds the class actually meets (e.g. all 15 in 7). Shrinking the round count moves material onto the last round rather than dropping it.
- **Add / remove / move material** — freely reassign topics between rounds; unlocks and canvas staging recompute to match. Unplaced topics wait in an Unassigned pool.
