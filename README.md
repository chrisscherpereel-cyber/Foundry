...
- **Games** — **run several games at once**, one per class / section / cohort. Each game has its own teams and advances on **its own round**, independently of the others. Create, rename, or delete games here, and pick the one you're managing with the **Active game** selector in the sidebar — every other Director page (Team Setup, Round Control, scoring, Cohort Overview…) then applies to that game. Students automatically play inside their own team's game, so two sections can be on different rounds simultaneously.
- **Team Setup** — three ways to create teams (all added to the **active game**):
  - **📥 Import a roster (.xlsx / .csv)** — upload a class contact list and the app builds games and teams automatically: each **Class + Section** becomes its own game, each **Team** number becomes a team, and the members are saved to that team's roster. Expected columns are **FirstName, LastName, PrimaryEmail, Class, Section, Team** (a **downloadable .xlsx template** is provided, and CSV works too). A preview shows the detected games/teams/members before you import; pick difficulty, territory mode, and founder-card style, then import. A single-game file can optionally be merged into the current game instead of a new one.
...
- **Round Briefing** — the starting point each round: learning objectives, concepts, tasks, and a **shaded completion checklist** ("To finish Round N — x/y complete") where amber items must still be done and green items are done. It also spells out **what must change this round** vs. **what can remain** (carried forward), and what's locked. Includes the full round-by-round arc.
  ...
  - **Sidebar progress tracker** — the team sidebar shows a live **"Round N to-do: x/y done"** bar with an expander listing exactly what's still open and on which tool (including anything carried over, **any unread Inbox messages**, and **any AI use still unverified** — both are to-do items), so from any page the team can see what's left this round.
...
- **Concept Check** — guarantees every concept introduced that round is covered, but **only asks a written question when it has to**. Most concepts are covered automatically by the **decisions the team makes in the tools** (e.g. building the Customer Profile covers "customer jobs, pains, gains"); the page lists those as ✅ covered-by-decision with no writing. Only concepts that need judgment and have no hands-on task (e.g. "behavior vs. opinion", "sunk-cost discipline") are checked here — with a quick **true/false understanding question** plus a one-sentence **application to the team's venture**. A concept is covered when the **true/false is correct** and the applied answer is a **real, on-topic sentence** — the pass rule is *complete + meaningful + at least one of {uses course concepts, relevant to their venture, evidence-based}*, shown as a live strength checklist. A **persistent status line always says exactly why it isn't covered yet** ("choose true/false", "the true/false isn't right yet", or what to add to the answer), so completion is never a mystery. If a round's concepts are all decision-covered, there's nothing to write. (This page sits just before the AI Assist Log in the sidebar.)
...
- **AI Assist Log** — turns AI *use* into AI *evaluation*, and makes it fast enough to take seriously. Logging is a **two-field quick-log** (the AI's **claim** + **how you'll check it**), with one-tap structured picks (claim type, whether a source was given) and an optional deeper AUDIT. **Every AI use gets a full — but fast — audit, and *where* it was used is always recorded.** A small **🔵 AI badge (a colored dot) sits next to every text-input area** (Founder & Opportunity, Canvases, Assumptions, Experiments, Evidence, VP Auction, Pivots, Concept Check, Decision Journal). Because each button knows its own field, pressing it **auto-records the exact field/section** (e.g. "Concept check: Behavior vs. opinion", "Canvas: Value Proposition Canvas", "Evidence Ledger") — no need to pick where. The form then captures the **prompt** and the **AI's response** (both required), **how the AI was used** (brainstorm / draft / summarize / critique / analyze / plan), then an **audit answered by dropdowns** — hidden assumptions, unsupported claims, sources/data, and how you'll verify — with free-text notes only if you want them. **Every field of a logged entry can be edited later** from the AI Assist Log page (✏️ Edit all fields), so you can come back after using AI in another tool and update it. **Verification stays the point:** link the claim to an assumption and it **auto-verifies** when you test it, or set the status with **one click** (✅/❌/✏️), and only verified evaluation (not usage) feeds the round score. **Any still-unverified AI use shows up on the sidebar to-do list** until it's checked. **Link a claim to an assumption and it auto-verifies**: when that assumption tests Supported/Refuted, the log flips to Verified/Rejected with no extra step. Status is set with **one-click buttons** (✅ Verified / ❌ Rejected / ✏️ Modified / ⏳ Unverified), and an **"N unverified" nudge** plus a verification-rate bar keep it honest. The round score's **AI-verification** component rewards the share you actually evaluated — **not** usage — and is neutral (`n/a`) for teams that don't use AI.
